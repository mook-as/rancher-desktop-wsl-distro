...
for repo_var in $(compgen -e); do
    ...
    gh_template='#{{ color "yellow" .number }}: {{ hyperlink .url .title }}'
    if gh pr view --json number,title,url --template "$gh_template" "$branch_name"; then
    ...
    # Commit the changes
    message="Bump ${short_name} from ${!version_var} to ${target_version}"
    git -c user.name="$GIT_AUTHOR_NAME" -c user.email="$GIT_AUTHOR_EMAIL" \
        commit --signoff --message "$message" versions.env
